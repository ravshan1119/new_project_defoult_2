...
//------------------Contacts ---------------
  ...
  static List<ContactModel> getContacts() {
    ...
    List<String> memoryContacts = _preferences?.getStringList("contacts") ?? [];
    return memoryContacts
        .map(
            (e) => ContactModel.fromJson(jsonDecode(e) as Map<String, dynamic>))
        .toList();
  }

  static Future<bool>? setContact(ContactModel contactModel) {
    if (_preferences == null) return null;
    List<ContactModel> contacts = getContacts();
    List<String> contactsAsList = contacts.map((e) => e.toString()).toList();
    contactsAsList.add(contactModel.toString());
    return _preferences!.setStringList("contacts", contactsAsList);
  }
}